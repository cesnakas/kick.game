...
if(!empty($t))
{
    ...
    $productId = intval(/*$_REQUEST["data"]["id"]*/$tags["id"]);
    $userId = $USER->GetID();
    $transactionId = $vpResponse["Transactions"][0]["TransactionId"];
    $payTransaction = CUser::GetList(($by = ""), ($order = ""), array("ID" => $userId), array("SELECT" => array("UF_PAY_TRANSACTION")))->Fetch()["UF_PAY_TRANSACTION"];

    if ($vpResponse["Success"] && $transactionId && $payTransaction != $transactionId && $productId) {
        ...
        CustomSubscribes::setUserSubscribeGroup($userId, $productId);
        if ($teamId)
        {
        ...
        $userOb = new CUser;
        $fields = array(
            "UF_PAY_TRANSACTION" => $transactionId,
        );
